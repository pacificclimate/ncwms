...
class AnimatedGifEncoder {
   ...
   public void addFrame(byte[] bgrPalette, byte[] indexedPixels, int transparentIndex) throws IOException {
      if (bgrPalette.length != 768) {
         throw new IllegalArgumentException("Palette must be 256 * 3 bytes long");
      } else if (this.width * this.height != indexedPixels.length) {
         throw new IllegalArgumentException("Image dimensions are inconsistent with length of array of pixel indices");
      } else if (transparentIndex > 255) {
         throw new IllegalArgumentException("transparentIndex must be less than 256");
      } else {
         if (transparentIndex >= 0) {
            this.transIndex = transparentIndex;
            this.transparent = new Color(0);
         }
